...
const ijirok = document.getElementById('IjirokToggle')

atkPreset.addEventListener('change', applyAtkPreset);
document.addEventListener('DOMContentLoaded', applyAtkBattery);
defPreset.addEventListener('change', applyDefPreset);
document.addEventListener('DOMContentLoaded', applyDefBattery);

ijirok.addEventListener('change', applyIjirok);
document.addEventListener('DOMContentLoaded', applyIjirok);
...
function applyIjirok() {
    const ijirokHeal = document.getElementsByClassName('Ijirok')

    if (ijirok.checked) {
        for (const option of ijirokHeal) {
            option.removeAttribute('disabled');
        }
    } else {
        for (const option of ijirokHeal) {
